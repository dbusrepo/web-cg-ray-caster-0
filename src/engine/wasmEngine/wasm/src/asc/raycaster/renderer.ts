...
@final @unmanaged class Renderer {
  private viewport: Viewport = changetype<Viewport>(NULL_PTR);
  private textures: SArray<Texture> = changetype<SArray<Texture>>(NULL_PTR);
  private mipmaps: SArray<BitImageRGBA> = changetype<SArray<BitImageRGBA>>(NULL_PTR);
  private frameColorRGBA: FrameColorRGBA = changetype<FrameColorRGBA>(NULL_PTR);
  private startFramePtr: PTR_T = NULL_PTR;
  private frameRowPtrs: SArray<PTR_T> = changetype<SArray<PTR_T>>(NULL_PTR);
  private lfloorSpanX: SArray<f32> = changetype<SArray<f32>>(NULL_PTR);
  private lfloorSpanY: SArray<f32> = changetype<SArray<f32>>(NULL_PTR);
  ...
  init(frameColorRGBA: FrameColorRGBA, viewport: Viewport, textures: SArray<Texture>, mipmaps: SArray<BitImageRGBA>): void {
    ...
    for (let y = 0 as u32; y <= viewport.Height; y++) {
      this.frameRowPtrs.set(y, this.startFramePtr + y * FRAME_STRIDE);
    }

    this.lfloorSpanX = newSArray<f32>(viewport.Height);
    this.lfloorSpanY = newSArray<f32>(viewport.Height);
    this.floorStepX = newSArray<f32>(viewport.Height);
    this.floorStepY = newSArray<f32>(viewport.Height);
  }

  public get FrameColorRGBA(): FrameColorRGBA {
  ...
  renderVert(wallSlices: SArray<Slice>, player: Player, map: Map, projYCenter: u32): void {
    ...
    const startFramePtr = this.startFramePtr;
...
let rendererAlloc = changetype<ObjectAllocator<Renderer>>(NULL_PTR);

function initRendererAllocator(): void {
  rendererAlloc = newObjectAllocator<Renderer>(1);
}

function newRenderer(): Renderer {
  if (changetype<PTR_T>(rendererAlloc) === NULL_PTR) {
    initRendererAllocator();
  }
  const renderer = rendererAlloc.new();
  return renderer;
}
