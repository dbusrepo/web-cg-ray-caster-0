...
function clearBg(
  ...
  const startOff: usize = rgbaSurface0ptr + start * FRAME_STRIDE;
  const endOff: usize = rgbaSurface0ptr + end * FRAME_STRIDE;
  ...
  // let ptr1 = startOff1;
  for (let ptr = startOff; ptr < endOff; ptr += BPP_RGBA) {
    store<u32>(ptr, color);
